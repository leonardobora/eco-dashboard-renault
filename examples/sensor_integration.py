...
def simulate_sensor_data():
    ...
    return {"power_meters": power_readings, "temperature_sensors": temp_readings}
...
if __name__ == "__main__":
    ...
    print(
        f"Sensores de temperatura: {len(simulated_data['temperature_sensors'])} leituras"
    )

    # Calcular consumo total simulado
    total_power = sum(reading.value for reading in simulated_data["power_meters"])
    avg_temp = sum(
        reading.value for reading in simulated_data["temperature_sensors"]
    ) / len(simulated_data["temperature_sensors"])
