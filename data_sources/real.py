...
class DatabaseDataSource(DataSourceInterface):
    ...
    def validate_data(self, reading: EnergyReading) -> bool:
        ...
        return (
            reading.power_consumption >= 0
            and reading.power_consumption <= 2000  # Max 2kW per device
            and reading.timestamp <= datetime.now()
        )
...
class RESTAPIDataSource(DataSourceInterface):
    ...
    def validate_data(self, reading: EnergyReading) -> bool:
        """Validate API data"""
        return reading.power_consumption >= 0 and reading.timestamp <= datetime.now()
        ...
        for source in self.sources:
            try:
                if not source.connect():
                    success = False
                    logger.warning(
                        f"Failed to connect data source: {type(source).__name__}"
                    )
            except Exception as e:
        ...
        for source in self.sources:
            if source.is_connected():
                try:
                    ...
                    logger.error(
                        f"Error getting devices from {type(source).__name__}: {e}"
                    )
        return all_devices
    ...
    def get_energy_readings(
        ...
        for source in self.sources:
            if source.is_connected():
                try:
                    all_readings.extend(
                        source.get_energy_readings(device_id, start_time, end_time)
                    )
                except Exception as e:
                    logger.error(
                        f"Error getting readings from {type(source).__name__}: {e}"
                    )
        return all_readings
        ...
        for source in self.sources:
            if source.is_connected():
                try:
                    ...
                    logger.error(
                        f"Error getting consumption from {type(source).__name__}: {e}"
                    )
        return total
